...
import { ShoppingBag, Check, Facebook, Instagram, MessageCircle, Star, Clock, Zap, Crown, Quote, User, CheckCircle2, Layers, ShieldCheck } from 'lucide-react';
...
const Services: React.FC<ServicesProps> = ({ onSelectPackage }) => {
  ...
  const renderPlatformIcon = (platform: string) => {
      switch(platform) {
          ...
          case 'All': 
          case 'Bundle': return <Layers className="w-5 h-5 text-amber-500" />;
          default: return null;
      }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {PACKAGES.map((pkg) => (
            <div 
                key={pkg.id} 
                className={`group relative rounded-[32px] backdrop-blur-md border transition-all duration-300 flex flex-col overflow-hidden ${
                    pkg.highlight 
                    ? 'bg-gradient-to-b from-slate-900 to-slate-950 border-accent-500 shadow-[0_0_40px_rgba(59,130,246,0.15)] transform lg:-translate-y-4 z-10 ring-1 ring-accent-500/50 scale-[1.02]' 
                    : 'bg-slate-900/40 border-white/5 hover:border-accent-500/30 hover:shadow-2xl hover:-translate-y-1'
                }`}
            >
                    ...
                    <div className="absolute -bottom-8 left-6">
                        <div className="relative">
                            <img 
                                ...
                                alt={pkg.platform}
                                className="w-16 h-16 rounded-full border-4 border-slate-900 object-cover shadow-lg"
                            />
                ...
                <div className="px-6 pb-6 pt-10 flex-1 flex flex-col relative">
                    {/* Highlight Badge */}
                    {pkg.highlight && (
                        <div className="absolute top-3 right-6 bg-gradient-to-r from-accent-500 to-accent-600 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-lg uppercase tracking-wider flex items-center gap-1 z-20 whitespace-nowrap border border-white/10">
                    ...
                    <div className="mb-6">
                        <h3 className="text-lg font-bold text-white mb-1 flex items-center gap-2">
                            {renderPlatformIcon(pkg.platform)}
                            {pkg.title} 
                        </h3>
                        <p className="text-xs text-slate-400 mb-3 font-medium">Professional Verification</p>
                        
                        <div className="flex items-baseline">
                            <span className="text-xl font-bold text-slate-500 align-top mr-1">$</span>
                            <span className={`text-5xl font-black tracking-tighter ${pkg.highlight ? 'text-transparent bg-clip-text bg-gradient-to-r from-accent-400 to-accent-500 drop-shadow-sm' : 'text-white'}`}>
                                {pkg.price}
                            </span>
                        </div>
                    ...
                    <div className="space-y-4 mb-8 flex-1 border-t border-white/5 pt-6">
                        <div className={`flex items-center gap-2 text-xs p-2.5 rounded-lg border justify-center font-bold ${
                            pkg.highlight 
                            ...
                            {pkg.features.map((feat, i) => (
                                <div key={i} className="flex items-start gap-3 text-xs text-slate-300">
                                    <div className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 border ${
                                        ...
                                        <Check className={`w-2.5 h-2.5 ${pkg.highlight ? 'text-accent-400' : 'text-slate-400'}`} />
                                    </div>
                    ...
                    <button 
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all relative overflow-hidden group ${
                            pkg.highlight 
                            ? 'bg-accent-600 text-white shadow-lg shadow-accent-500/20 hover:scale-[1.02] hover:bg-accent-500 border border-accent-400'
                            : 'bg-white/5 border border-white/10 text-white hover:bg-white/10 hover:border-white/20'
                        }`}
                    >
                        {pkg.highlight && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>}
      ...
      <div className="mt-24 pt-16 border-t border-white/5">
        <div className="text-center mb-12 animate-in fade-in slide-in-from-bottom-4 space-y-4">
        ...
        {/* Responsive Carousel/Grid: Flex on mobile (carousel), Grid on desktop */}
        <div className="flex overflow-x-auto pb-8 gap-6 md:grid md:grid-cols-3 md:pb-0 md:gap-8 snap-x snap-mandatory scrollbar-hide">
            {TESTIMONIALS.map((t, i) => (
                <div 
                    key={t.id} 
                    className="min-w-[300px] md:min-w-0 snap-center bg-slate-900/40 backdrop-blur-md p-8 rounded-[32px] border border-white/5 shadow-xl relative group hover:-translate-y-2 transition-all duration-300 overflow-hidden flex flex-col"
                ...
                >
                    ...
                    <div className="relative z-10 flex-1 flex flex-col">
                        ...
                        <p className="text-slate-300 text-sm leading-relaxed mb-8 font-medium italic flex-1">
                            "{t.content}"
                        </p>
                        
                        <div className="flex items-center gap-4 mt-auto">
                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-slate-800 to-slate-900 p-[2px] border border-white/10 shrink-0">
